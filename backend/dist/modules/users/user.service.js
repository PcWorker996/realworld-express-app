...
export const createUser = async (input) => {
    ...
    const result = await pool.query(`INSERT INTO users (username, email, password_hash) 
     VALUES ($1, $2, $3) 
     RETURNING id, username, email, created_at, updated_at`, [username, email, hashedPassword]);
...
export const loginUser = async (input) => {
    ...
    // Find user by email
    const result = await pool.query(`SELECT id, email, username, password_hash, bio, image 
     FROM users 
     WHERE email = $1`, [email]);
    const user = result.rows[0];
...
export const getUser = async (userId, token) => {
    const result = await pool.query(`SELECT id, email, username, bio, image 
     FROM users 
     WHERE id = $1`, [userId]);
    const user = result.rows[0];
    if (!user) {
 ...
 * @throws Error - Internal server error
 */
export const updateUser = async (userId, input, token) => {
    const updates = [];
    const values = [];
    let paramCount = 1;
    if (input.email) {
    ...
    const result = await pool.query(`UPDATE users
     SET ${updates.join(", ")}
     WHERE id = $${paramCount}
     RETURNING id, email, username, bio, image`, values);
    const user = result.rows[0];
    if (!user) {
        throw new Error("User not found");
    }
